...
export const interact = async (req: Request, res: Response) => {
    ...
    try {
      ...
      const getActionData = (body: any) => {
        const action = body.action || body.request;
        return {
          type: action?.type,
      ...
      const actionData = getActionData(req.body);

      // Set input attributes
      try {
        if (actionData.type === 'launch') {
          parentSpan.setAttributes({
            [SemanticConventions.OPENINFERENCE_SPAN_KIND]: OpenInferenceSpanKind.CHAIN,
            [SemanticConventions.INPUT_VALUE]: 'LAUNCH',
            [SemanticConventions.INPUT_MIME_TYPE]: MimeType.TEXT,
          });
        } else if (actionData.type?.startsWith('path-')) {
          parentSpan.setAttributes({
            [SemanticConventions.OPENINFERENCE_SPAN_KIND]: OpenInferenceSpanKind.CHAIN,
            [SemanticConventions.INPUT_VALUE]: JSON.stringify({ messages: [
              {
        ...
        } else if (actionData.type?.startsWith('intent')) {
          parentSpan.setAttributes({
            [SemanticConventions.OPENINFERENCE_SPAN_KIND]: OpenInferenceSpanKind.CHAIN,
            [SemanticConventions.INPUT_VALUE]: JSON.stringify({ messages: [
              {
                "role": "user",
                "content": actionData.query || ""
              }
            ]}),
            [SemanticConventions.INPUT_MIME_TYPE]: MimeType.JSON,
          });
